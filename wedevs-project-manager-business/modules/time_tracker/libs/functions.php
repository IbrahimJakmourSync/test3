...
function pm_pro_timetracker_before_create_subtask( $data, $request ) {
    ...
    if( ! pm_pro_has_subtask_estimated_time( $request ) ) {
        $data['estimation'] = 0;
    } else {
        $data['estimation'] = pm_pro_get_hour_to_minute( $request['estimation'] );
...
function pm_pro_timetracker_before_update_subtask( $data, $request, $sub_task ) {
    $request = $request->get_params();
    ...
    if ( isset( $request['estimation']) && empty( $request['estimation'] ) ) {
        $data['estimation'] = 0;
    };

    if( ! pm_pro_has_subtask_estimated_time( $request ) ) {
        if ( ! $status && isset( $request['estimation'] ) && empty( $request['estimation'] ) ) {
            $data['estimation'] = 0;
        }
    } else {
        $data['estimation'] = pm_pro_get_hour_to_minute( $request['estimation'] );
